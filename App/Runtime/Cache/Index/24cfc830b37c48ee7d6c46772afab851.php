...
<div class="main">
    ...
    <div class="main_right">
        ...
        <div class="content">
            <div class="list_main">
                <div class="title">
                    <form action="<?php echo U('article_list');?>" method="get">
                        <p>
                           <label>文章标题</label><input type="text" name="title" value="<?php echo ($_GET['title']); ?>" class="txt">
                           <label>发布日期</label><input type="text" name="start" value="<?php echo ($_GET['start']); ?>" class="timer">至<input type="text" name="end" value="<?php echo ($_GET['end']); ?>" class="timer">
                        </p>
                        <p>
                           ...
                           <select name="type">
                               ...
                               <?php $_result=getData('article_type','all','','','sort');if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$t): $mod = ($i % 2 );++$i; if($t['id'] == $_GET['type']): ?><option value="<?php echo ($t["id"]); ?>" selected="selected"><?php echo ($t["name"]); ?></option>
                                       <?php else: ?>
                           ...
                           <a href="<?php echo U('article_add');?>" class="two">添加文章</a>
                ...
                <table>
                    <tbody>
                        ...
                        <?php $_result=getData('article','all',$where,$limit,'id desc');if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$a): $mod = ($i % 2 );++$i;?><tr>
                                <td><input type="checkbox" name="ck[]" value="<?php echo ($a["id"]); ?>"></td>
                                <td><?php echo ($a["title"]); ?></td>
                                <td><?php echo getArticleType($a['type']);?></td>
                                <!--<td><?php echo ($a["keyword"]); ?></td>-->
                                <td><?php if($a['show'] == 1): ?>显示<?php else: ?>隐藏<?php endif; ?></td>
                                <td><?php echo ($a["time"]); ?></td>
                                <td>
                                    <a href="<?php echo U('article_edit','id='.$a['id']);?>" class="update">编辑</a>
                                    <a href="javascript:;" onclick="delList(<?php echo ($a["id"]); ?>)" class="del">删除</a>
                                </td>
...
//            window.location.href = "/admin/dellist/data/article/list/"+obj+".html?re=<?php echo U('article_list');?>";
            window.location.href = "<?php echo U('dellist');?>?data=article&list="+obj+"&re=<?php echo U('article_list');?>";
        }
    }
    function delCheck(){
        var arr = new Array();
