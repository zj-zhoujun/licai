...
<div class="content">
    <form action="<?php echo U('system_activity');?>" method="post" enctype="multipart/form-data">
        <div class="line">
            <label>活动图片<br/><span style="color:#f00;font-size: 12px;">300*200像素,PNG格式</span></label>
            <div class="uploads">
                <img src="/Public/uploads/activity.png?k=<?php echo rand(1,99999);?>" id="portrait">
                <input type="file" class="file" name="img"  accept="image/png" onchange="showPreview(this)">
            </div>
        </div>

        <div class="line">
            <label>活动链接：</label>
            <input type="text" name="activity_url" value="<?php echo getInfo('activity_url');?>">
        </div>
        <div class="line">
            <label>是否开启活动：</label>
            <select name="activity_status">
                <option value="1" <?php if(getInfo('activity_status') == 1): ?>selected="selected"<?php endif; ?>>开启</option>
                <option value="0" <?php if(getInfo('activity_status') == 0): ?>selected="selected"<?php endif; ?>>关闭</option>
            </select>
        </div>
        <div class="button">
