...
function redirect($url, $time=0, $msg='') {
    $start = '<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
        ...
        <div style="text-align: center;position: absolute;margin: auto;top: 0;left: 0;right: 0;bottom: 0;line-height: 40px;width: 240px;height:40px;background-color: rgb(255,252,209);color:#666;font-weight:bold;font-size: 14px;border-radius: 2px;border: 1px solid #ccc;">';
    $end = "</div>";
    //多行URL地址支持
    $url = str_replace(array("\n", "\r"), '', $url);
    if (empty($msg))
//        $msg    = $start."系统将在{$time}秒之后自动跳转到{$url}！".$end;
        $msg    = $start."系统将在{$time}秒之后自动跳转！".$end;
    if (!headers_sent()) {
        // redirect
        if (0 === $time) {
            header('Location: ' . $url);
